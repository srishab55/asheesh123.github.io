function doNext(user) {
  ...
  function submitForm(e){
      ...
      writeUserData(user,ckts, archi, origin,amalg,deception,expo,total_fee);
  }
}

function writeUserData(user,ckts, archi, origin,amalg,deception,expo,total_fee) {

        firebase.database().ref('eee').child(user.uid+"").set({
            username: user.email,
...
function fetchData(user){
      var leadsRef = firebase.database().ref('eee/'+user.uid);
      var flag=true;
