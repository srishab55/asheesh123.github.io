function doNext(user) {
  ...
  function submitForm(e){
      ...
      writeUserData(user,hand,art,artathon,paint,adv,photo,mannequin,foto,total_fee);
  }
}

function writeUserData(user,hand,art,artathon,paint,adv,photo,mannequin,foto,total_fee) {

        firebase.database().ref('pnp').child(user.uid+"").set({
            username: user.email,
            HAND_PAINTING:hand,
            ART_IN_LINE:art,
            ART_A_THON:artathon,
...
function fetchData(user){
      var leadsRef = firebase.database().ref('pnp/'+user.uid);
      var flag=true;
