function doNext(user) {
  ...
  function submitForm(e){
      ...
      writeUserData(user,la,float,ep,crack,pop,total_fee);
  }
}

function writeUserData(user,la,float,ep,crack,pop,total_fee) {
      firebase.database().ref('civil').child(user.uid+"").set({
          username: user.email,
          LA_TECQUILA: la,
          FLOATCRETE:float,
          EPSIDA:ep,
...
function fetchData(user){
      var leadsRef = firebase.database().ref('civil/'+user.uid);
      var flag=true;
