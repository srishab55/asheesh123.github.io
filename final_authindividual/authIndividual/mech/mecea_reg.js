function doNext(user) {
  ...
  function submitForm(e){
      ...
      writeUserData(user,ammc,giss,marc,robo,wor,total_fee);
  }
}

function writeUserData(user,ammc,giss,marc,robo,wor,total_fee) {

        firebase.database().ref('mech').child(user.uid+"").set({
            username: user.email,
...
function fetchData(user){
      var leadsRef = firebase.database().ref('mech/'+user.uid);
      var flag=true;
