...
function doNext(user) {
  ...
  function submitForm(e){
      ...
      writeUserData(user,lect,eq,ph,bd,qg,total_fee);
  }
}

function writeUserData(user,lect,eq,ph,bd,qg,total_fee) {
  firebase.database().ref('chem').child(user.uid+"").set({
      username: user.email,
      LECTURES:lect,
      EXQUIZITE:eq,
      CHEM_PHEONIX:ph,
...
function fetchData(user){
      var leadsRef = firebase.database().ref('chem/'+user.uid);
      var flag=true;
